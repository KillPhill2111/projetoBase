...
namespace HelperStockBeta.Domain.Entities
{
    public sealed class Product :  Entity
    {
        ...
        {
            DomainExceptionValidation.When(id < 0, "Invalid negative value for id.");
        ...
        private void ValidationDomain(string name, string description,decimal price, int stock,string image )
        {
            ...
            DomainExceptionValidation.When(name.Length<3,
                "Invalid short name, minimum 3 characteres.");
            DomainExceptionValidation.When(string.IsNullOrEmpty(description),
                "Invalid description, description is required");
            DomainExceptionValidation.When(description.Length <5,
                "Invalid short descriptor, minimum 5 characteres"); 
            DomainExceptionValidation.When(price <0 , 
                "Invalid negative value for price");
            DomainExceptionValidation.When(stock <0,
                "Invalid negative value for stock");
